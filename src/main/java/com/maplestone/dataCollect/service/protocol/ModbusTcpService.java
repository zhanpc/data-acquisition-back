...
@Slf4j
@Service
public class ModbusTcpService implements ProtocolHandler {
    ...
    public boolean connect(String connectionId, Integer stationId, String host, int port) {
        ...
        try {
            ...
            log.info("Modbus TCP 连接成功: {}:{}", host, port);
            return true;
        } catch (Exception e) {
            log.error("Modbus TCP 连接失败: {}", e.getMessage());
            return false;
        }
    }
    ...
    public int[] readHoldingRegisters(String connectionId, int unitId, int address, int quantity) throws Exception {
        TCPMasterConnection connection = connectionMap.get(connectionId);
        if (connection == null) {
    ...
    public void disconnect(String connectionId) {
        ...
        if (connection != null) {
            ...
            log.info("Modbus TCP 连接已断开: {}", connectionId);
        }
    }
